...
if [[ ! -f ./reference/human_g1k_v37.fasta ]]; then
	mkdir ./reference 
...
for iteration in 1 2; do
  ...
  *)
  ...
  for chrNum in ${chromosomes[@]}; do
    ...
    # read-only phasing
    out=${out_raw/.vcf.gz/_SmartPhase_read-only.tsv}
    log=${out/.tsv/.log}
    echo "Running SmartPhase on chr$chrNum for $sample trio in read-only mode..."
    java -jar ../smartPhase.jar -g $merged_gene_bed -a $vcf -p $child -r $bam -m 60 -o results/$out -v -x > results/$log 2>&1
    perl ./extract_SmartPhase_stats.pl ./results $sample $child "chr$chrNum" "read-only" 10 0.34 >> $sp_outtable
    
    # read and trio phasing
    out=${out_raw/.vcf.gz/_SmartPhase_read+trio.tsv}
    log=${out/.tsv/.log}
    echo "Running SmartPhase on chr$chrNum for $sample trio in read+trio mode..."
