...
Download sample data and jar and execute like this:
...
A comma seperated list of integers indicating the cut-off mapping quality value for reads in the respective file given in the reads flag (-r). For example, if the arguments passed to -r are file1,file2 and the arguments passed to -m are 255,60 this would indicate that only those reads in file1 with a mapping quality of at least 255 are to be considered and only those reads with a mapping quality of at least 60 in file2 are to be considered (in this case most likely because file1 contains reads generated through RNAseq and file2 through DNAseq)
...
When running an analytic mode, the filtered variants file (-f) will only consist of pre-selected variant pairs for which the phase should be determined. Only tab and comma seperated files are accepted here. The first field should contain the patient id as given in the -p flag. The second and third field are the two variants whose phase should be determined. Each variant field must follow the following pattern <chromisomeNumber>-<startPosition>-<referneceAlleleString>-<alternateAlleleString>. For example: chr1-143555-G-A.
